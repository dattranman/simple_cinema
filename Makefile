...
default: 
	...
	@echo "    run: Running as daemon service"
...
.PHONY: run
## run is a command to run the application locally
run:
	@docker compose -f docker-compose.yaml up -d simple_cinema

.PHONY: build-docker
## build-docker is a command to build the application locally
build-docker:
	@docker build -t simple_cinema:$(VERSION) .
	@docker compose -f docker-compose.yaml build simple_cinema
...
.PHONY: migrate
## migrate is a command to run the migration
migrate:
	@docker compose -f docker-compose.yaml up -d simple_cinema_goose
